...
class EmailWorker(QThread):
    ...
    archivo_enviado = pyqtSignal(int, int) # (enviados, total)
    terminado = pyqtSignal(bool, str)

    def __init__(self, archivos, destino):
        super().__init__()
        self.archivos = archivos
        self.destino = destino
        self.origen = settings.value("Compartir/correo", "")
        self.password = settings.value("Compartir/password", "")
    ...
    def run(self):
        try:
            ...
            for archivo in self.archivos:
                ...
                # Notificar archivo enviado.
                self.archivo_enviado.emit(enviados, total_archivos)
